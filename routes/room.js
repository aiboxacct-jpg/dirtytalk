...
router.get('/', async (req, res) => {
  const rows = await db.all(
    `SELECT gm.*, u.verified AS creator_verified, u.handle AS creator_handle, u.avatar_url AS creator_avatar, u.gender AS creator_gender
  ...
  const creators = await db.all(
    'SELECT id, name, handle, verified, avatar_url, gender FROM users ORDER BY COALESCE(online_at, created_at) DESC LIMIT 12'
  );
  res.render('room', {
    title: 'Room',
